...
class TribeBuildingTest {
    ...
    void shouldApplyCorrectEffect() {
        ...
        player.getTribe().addCardToTribe(new Inventor(1, 0, InventorSymbol.BOAT));
        example = new TribeBuilding(1, 0, 1, 1, new Effects.FoodOnInventorSymbolPair());
        example.tribeBuildingEffect(player);
        assertEquals(3, player.getFood());

        player = new Player("2");
        player.getTribe().addCardToTribe(new Builder(1, 0, 2, 3));
        example = new TribeBuilding(1, 0, 1, 1, new Effects.BonusShamanStars());
        example.tribeBuildingEffect(player);
        ...
        player.getTribe().addCardToTribe(new Artist(1, 0));
        example = new  TribeBuilding(1, 0, 1, 1, new Effects.SustenanceDiscountByCharacterType(CharacterType.ARTIST));
        example.tribeEntranceEffect(player);
        ...
        player.getTribe().addCardToTribe(new Inventor(1, 0, InventorSymbol.BOAT));
        example = new  TribeBuilding(1, 0, 1, 1, new Effects.FoodOnSet());
        example.tribeBuildingEffect(player);
